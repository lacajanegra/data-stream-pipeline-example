...
class CustomScript extends BootScript {
  ...
  boot(next) {
    ...
    logger.info(`[queue] Consumer id:    ${chalk.bold.green(clientId)}`);
    logger.info(`[queue] Queue group id: ${chalk.bold.green(queueGroup)}`);
    logger.info(emoji.emojify('[queue] Subscribed to events:'));
    eventsToSubscribe.forEach((eventName) => {
      logger.info(emoji.emojify(`[queue] - ${chalk.bold.green(eventName)}`));
    });
    ...
    expr.whenTrue(cnx.indexOf(',') > 0, () => {
      ...
      logger.info(chalk.green('[queue] running on clustered mode'));
    ...
    client
      ...
      .on('queue:state_changed', (newState) => {
        ...
        logger.info(`[queue] Status changed: ${state}`);
